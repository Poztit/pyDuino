...
class LiquidCrystal():
    def __init__(self, RSIn, EIn, D4In, D5In, D6In, D7In):
        ...
        self.D6 = D6In
        self.D7 = D7In
        self.dataPin = [self.D7, self.D6, self.D5, self.D4]
        # a l'envers pour être dans même ordre que bits
        self.configDisplay = 0x00  # variable interne drapeau affichage
        # se combine avec LCD_DISPLAYCONTROL
        self.configMain = 0x00  # variable interne drapeau config generale
        # se combine avec LCD_FUNCTIONSET
        self.configEntryMode = 0x00  # variable interne drapeau config generale
        # se combine avec LCD_ENTRYMODESET
    ...
    def begin(self, colonnesIn, lignesIn):
        ...
        # ou bit à bit ( | ) permet mise à 01
        # des bits voulus laissant les autres inchangés
    ...
    def cmd4Bits(self, cmdIn):
        ...
        for i in range(4, 8):  # les 4 bits de poids faible
            ...
            # met la broche dans l'état voulu
            if cmdBin[i] == "1":
                digitalWrite(self.dataPin[i - 4], HIGH)
            else:
                digitalWrite(self.dataPin[i - 4], LOW)
    ...
    def pulseEnable(self):  # impulsion Enable de validation
        ...
        delayMicroseconds(1)
        digitalWrite(self.E, LOW)  # Enable à LOW
        delayMicroseconds(100)  # le temps d'utilisation des data
    ...
    def write(self, chaineIn):  # affiche la chaîne recue
        ...
        for c in chaine:
            ...
            self.data4Bits(ord(c))  # affiche l'ascii correspondant
            # attention envoyer sous forme d'un str
    ...
    def noDisplay(self):
        ...
        self.configDisplay = self.configDisplay & ~ LCD_DISPLAYON
        # modifie flag display en se basant sur flag ON ...
        # print bin(self.configDisplay) # debug
        self.cmd4Bits(LCD_DISPLAYCONTROL | self.configDisplay)
    ...
    def noCursor(self):
        # print bin(self.configDisplay) # debug
        self.configDisplay = self.configDisplay & ~ LCD_CURSORON
        # modifie flag display en se basant sur flag ON ...
        # print bin(self.configDisplay) # debug
        self.cmd4Bits(LCD_DISPLAYCONTROL | self.configDisplay)
    ...
    def noBlink(self):
        # print bin(self.configDisplay) # debug
        self.configDisplay = self.configDisplay & ~ LCD_BLINKON
        # modifie flag display en se basant sur flag ON ...
        # print bin(self.configDisplay) # debug
        self.cmd4Bits(LCD_DISPLAYCONTROL | self.configDisplay)
    ...
    def locate(self, ligneIn, colonneIn):
        # position au format ligne,colonne avec 1,1 pour 1er caractère
...
class Servo():
    def __init__(self):
        ...
        self.impulsMin = 550
        self.impulsMax = 2350
        pwmMin = (255 * self.impulsMin / (1000000.0 / freqBasePWM)) + 0.5
        self.pwmMin = int(pwmMin)  # 255*impulsMin/T
    ...
    def detach(self):
        setFrequencyPWM(self.pin, 520)  # Frequence par défaut
        # broche mise à 0 auto
