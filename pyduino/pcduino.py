...
# ### Pour PWM ###
# identifiant broches PWM
PWM = [3, 5, 6, 9, 10, 11]  # List pour acces par indice
PWM0, PWM1, PWM2 = PWM[0], PWM[1], PWM[2]
PWM3, PWM4, PWM5 = PWM[3], PWM[4], PWM[5]
...
initPwmFlag = [False, False, False, False, False, False]
# Flags init Freq PWM - false tant que pas initialisation freq PWM
...
def pinMode(pin, mode):
    ...
    file = open(pathMode + "gpio" + str(pin), 'w')
...
def digitalWrite(pin, state):
    ...
    file = open(pathState + "gpio" + str(pin), 'w')
    # Ouvre le fichier en ecriture
    file.write(state)
    file.close()
...
def digitalRead(pin):
    ...
    file = open(pathState + "gpio" + str(pin), 'r')
...
def pulseOut(pin, duree):  # crée une impulsion sur la broche de durée voulue
    ...
    # mais a pour but d'éviter la saisie des 3 instructions
    # en la remplaçant par une seule
...
def analogRead(pinAnalog):
    ...
    # pin = int(pinAnalog)  # pin est un int entre 0 et 5
    # utilisation identifiant predefini possible
...
def analogReadmV(*args):
    ...
    # 2 formes :
    # pinAnalog
    # pinAnalog, rangeIn, mVIn où range est la resolution
    # et mV la tension max = qui renvoie 4095
    ...
    if len(args) == 1:  # forme pinAnalog
        ...
        # forme pinAnalog, range, mV où range est la resolution
        # et mV la tension max = qui renvoie 4095
        pinAnalog = args[0]
        rangeIn = args[1]
        mVIn = args[2]
        mesure = analogRead(pinAnalog)  # mesure sur la broche
        mesure = rescale(mesure, 0, rangeIn, 0, mVIn)
        # en pratique, la mesure 4095 est atteinte à 3V..
    return mesure
        ...
        if pinPWMIn == 5 or pinPWMIn == 6:  # si broches 5 et 6
            ...
            if frequencePWMIn >= MIN_PWMTMR_FREQ and frequencePWMIn <= MAX_PWMTMR_FREQ:  # si freq entre 126 et 2000Hz
                ...
                if ret < 0:
                    ...
                    return
        if fd:
            fd.close()  # ferme fichier si existe
    else:
        raise "Broche non autorisee pour PWM"
...
def analogWriteHardware(pinPWMIn, largeurIn):
    ...
    if (pinPWMIn == 3 or pinPWMIn == 5 or pinPWMIn == 6 or pinPWMIn == 9 or pinPWMIn == 10 or pinPWMIn == 11) and largeurIn >= 0 and largeurIn <= MAX_PWM_LEVEL:
        ...
        print("Broche non autrisee pour PWM")
